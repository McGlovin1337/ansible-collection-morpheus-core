...
def instance_filter(morpheus_api: MorpheusApi, module_params: dict, key_filter: list = None) -> list:
    ...
    if len(response) > 1:
        ...
        return {
            'none': [],
            'first': [response[0]] if key_filter is None else [mf.dict_filter(response[0], key_filter)],
            'last': [response[-1]] if key_filter is None else [mf.dict_filter(response[-1], key_filter)],
            'all': [instance if key_filter is None else mf.dict_filter(instance, key_filter) for instance in response]
        }.get(match_name)

    return [mf.dict_filter(instance, key_filter) if key_filter is not None else instance for instance in response]
